...
typedef std::function<micro::uMeasure(const actions::proto::Step&, units::Unit*)>
    CostCalculator;

// Default CostCalculator that always returns 0.
micro::uMeasure ZeroCost(const actions::proto::Step& step, units::Unit* unit);
// Default CostCalculator that always returns 1.
micro::uMeasure OneCost(const actions::proto::Step& step, units::Unit* unit);
