...
constexpr micro::Measure kMinPracticalScale = micro::kOneInU / 10;
...
void LocalProfitMaximiser::SelectCandidate(
  ...
  for (const auto& prod_info : candidates) {
    micro::Measure scale_u = prod_info->max_scale_u();
    ...
    *reject = *prod_info;
    ...
    micro::Measure reven_u = context->market->GetPriceU(
        prod_info->expected_output(), prod_info->step_info_size());
    micro::Measure cost_u = 0;
    micro::Measure current_scale_u = scale_u;
    for (int step = 0; step < prod_info->step_info_size(); ++step) {
      ...
      micro::Measure lowest_var_cost_u = micro::kMaxU;
      micro::Measure best_scale_u = current_scale_u;
      int best_var = 0;
      for (int var = 0; var < step_info->variant_size(); ++var) {
        const industry::decisions::proto::VariantInfo& var_info =
            step_info->variant(var);
        micro::Measure var_cost_u = micro::MultiplyU(
        ...
        micro::Measure var_scale_u = var_info.possible_scale_u();
        micro::Measure scale_loss_u = current_scale_u - var_scale_u;
        if (scale_loss_u > 0) {
    ...
    micro::Measure profit_u = reven_u - cost_u;
    if (profit_u <= 0) {
